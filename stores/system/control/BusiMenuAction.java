...
public class BusiMenuAction extends BaseAction {
	...
	public String queryMenuForPage(){
		...
		try {
			if(Tool.isNotEmpty(mid)&&"0".equals(mid)){
				...
				List<SyMenuList> mlist=menuService.queryBusiMenu(params);//.queryMenu(params);
				...
				List wfNodes;
				wfNodes = menuService.queryBusiSonById(mid);//.querySonById(mid);
				for(int i=0;i<wfNodes.size();i++){  
	                ...
	                if("30".equals(wfNodesTo.getMenuType().toString())){//function button;
	                	...
	                	List wfList=menuService.queryBusiSonById(wfNodesTo.getMenuId().toString());//.querySonById(wfNodesTo.getMenuId().toString());
	                	if(wfList.size()>0){
	                		jsonstr=jsonstr+ 
		                	"{\n" +  
			                "    \"id\":"+wfNodesTo.getMenuId()+",\n" +   
	...
	public String queryForLeftMenu(){
		...
		try {//splice jsonStr;
			...
			//query role de menu
			List<RoleMenuBean> list=menuService.queryBusiRelation(params1);//.queryRelation(params1);
...
//                "    \"text\":\"<a href='javaScript:void(0)' target='rightframe'>"+menu.getMenuName()+"</a>\",\n" +  "    \"state\":\"closed\"\n" +     "  },";
				...
				List<SyMenuList> sonNode=menuService.queryBusiSonById(menu.getMenuId().toString());//.querySonById(menu.getMenuId().toString());
				if(sonNode.size()>0){
	...
	public String appendChild(String jsonstr,List child) throws ServiceException{
		for (int i = 0; i < child.size(); i++) {
			...
			if("20".equals(menu.getMenuType().toString())){
				...
				jsonstr+="<div title='"+menu.getMenuName()+"' style='padding: 10px;'>";
				List sonlist=menuService.queryBusiSonById(menu.getMenuId().toString());//.querySonById(menu.getMenuId().toString());
				if(sonlist.size()>0){ 
